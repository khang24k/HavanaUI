      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((item, idx) => (
            <div 
              key={idx} 
              className="bg-white p-8 rounded-xl border border-zinc-200 shadow-sm text-center space-y-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-md"
            >
              <div className="w-12 h-12 rounded-xl bg-sky-50 text-sky-600 border border-sky-100 flex items-center justify-center mx-auto font-bold">
                {item.icon}
              </div>
              <div className="text-4xl font-bold text-zinc-900 tracking-tight">{item.number}</div>
